...
// Processes critical CSS, to be included in head.html.
gulp.task('build:styles:critical-post', function() {
    return sass(paths.sassFiles + '/critical-post.scss', {
        style: 'compressed',
        trace: true,
        loadPath: [paths.sassFiles]
    }).pipe(postcss([ autoprefixer({ browsers: ['defaults'] }) ]))
...
// Builds all styles.
gulp.task('build:styles', ['build:styles:main', 'build:styles:critical', 'build:styles:critical-post']);

gulp.task('clean:styles', function(callback) {
    del([paths.jekyllCssFiles + 'main.css',
        paths.siteCssFiles + 'main.css',
        '_includes/critical.css', '_includes/critical-post.css'
    ]);
    callback();
});
...
gulp.task('serve', ['build:local'], function() {
    ...
    gulp.watch('_assets/styles/**/*.scss', ['build:styles', 'build:jekyll:watch']);
